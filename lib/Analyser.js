...
class Analyser extends Transform {
  ...
  _transform(chunk, encoding, cb) {
    ...
    const {
      push, fftSize, inputBuffer, float,
      getFloatFrequencyData, getByteFrequencyData, outputArray,
    } = this
    const { length, copy } = chunk
    ...
    for (let byteToCopy = fftSize * SIZE_OF_FLOAT_32; readIndex < length; readIndex += byteToCopy) {
      ...
      copy(inputBuffer, readIndex, writeIndex, writeIndex + byteToCopy)
      writeIndex += byteToCopy
      if (writeIndex >= fftSize) {
        writeIndex = 0
      }
      if (float) {
        getFloatFrequencyData(outputArray)
      } else {
        getByteFrequencyData(outputArray)
      }
      push(outputArray.buffer)
    }

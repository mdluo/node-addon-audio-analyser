...
class Analyser extends Transform {
  constructor(options = {}) {
    ...
    this.outputArrayLength = (this.fftSize / 2) * (this.float ? SIZE_OF_FLOAT_32 : 1)
  ...
  _transform(chunk, encoding, cb) {
    const {
      ...
      outputBuffer, outputArrayLength, outputCounter,
    } = this
    ...
    while (this.inputQueue.length >= inputBufferLength) {
      ...
      this.getFrequencyData(outputBuffer.slice(
        outputCounter * outputArrayLength,
        (outputCounter + 1) * outputArrayLength
      ))
